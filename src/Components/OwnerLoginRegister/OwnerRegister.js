...
const OwnerRegister = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (response?.user?.email) {
      await axios
        .post("/api/v1/owner/register", { email: response?.user?.email })
  ...
  return (
    <div className="flex gap-20 min-h-screen items-center justify-center bg-gray-100 p-10">
      <div className="p-10 bg-white rounded-lg shadow-md sm:w-96">
        <h2 className="text-2xl font-bold mb-5 text-center">Owner Register</h2>
        <form onSubmit={handleSubmit}>
          ...
          {userEmailCheck ? (
            ...
            >
              This account is already associated to an user
            </button>
          ) : (
            <button
              type="submit"
        ...
        <p className="mt-4 text-sm text-center text-gray-500">
          ...
          <Link to="/owner-login" className="underline ml-1">
            Login here
          </Link>
        </p>
      </div>
